...
public:
  ...
  void output_element() {
    std::cout << "Def Line:"<<defuse_ln.first << " " << "Def Column:"<<defuse_cl.first << "\n";
    std::cout << "Use Line:"<<defuse_ln.second << " " << "Use Column:"<<defuse_cl.second << "\n";
    std::cout << "Belong to Function: " << func_name << "\n";
    if (defuse_stmt.first != nullptr)
      std::cout << "Def Stmt:"<<defuse_stmt.first->getStmtClassName() << "\n";
    if (defuse_stmt.second != nullptr)
      std::cout << "Use Stmt:"<<defuse_stmt.second->getStmtClassName() << "\n";
  ...
  */
  ...
  void output_all(SourceManager *scm) {
    ...
    for (auto i = du.begin(); i != du.end(); ++i) {
      std::cout << "Path index:"<<(*i).first.first << "\n";
      std::cout << "Location:"<<(*i).first.second.printToString(*scm);
      std::cout << "\n";
